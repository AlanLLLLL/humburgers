<template>
    <div class="cashFlowAdd">
       <el-form :model="cashFlowData" :rules="rules" ref="cashFlowForm" label-width="100px" class="demo-cashFlowForm">
            ...
            <el-form-item>
                <el-button class="editBtn" type="primary" @click="goBack">返回</el-button>
                <el-button class="editBtn" type="primary" @click="submitForm('cashFlowForm')">添加</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
...
export default {
  ...
  methods:{
      getCashData(){
          ...
          this.$axios.get(`/api/cashflows/${this.$route.params.id}`)
            .then(res => {
      ...
      submitForm(formName){
          ...
          if (valid) {
              console.log(this.$route.params.id)
            this.$axios.post(`/api/cashflows/${this.$route.params.id}`,this.cashFlowData)
                .then((res) => {
                    this.$message({
                    message: "编辑成功！",
                    type: "success"
